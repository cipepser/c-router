  ...
  for (i = 0; i < Ip2Macs[deviceNo].no; i++) {
    ...
    if (ip2mac->addr == addr) {
      ...
      if (hwaddr != NULL) {
        ...
        if ((ip2mac->flag == FLAG_OK &&
             now - ip2mac->lastTime > IP2MAC_TIMEOUT_SEC) ||
            (ip2mac->flag == FLAG_NG &&
             now - ip2mac->lastTime > IP2MAC_NG_TIMEOUT_SEC)) {
          FreeSendData(ip2mac);
  ...
  if (freeNo == -1) {
    ...
    if (no >= Ip2Macs[deviceNo].size) {
      if (Ip2Macs[deviceNo].size == 0) {
        ...
        Ip2Macs[deviceNo].data =
            (IP2MAC *)malloc(Ip2Macs[deviceNo].size * sizeof(IP2MAC));
      } else {
        Ip2Macs[deviceNo].size += 1024;
        Ip2Macs[deviceNo].data =
            (IP2MAC *)realloc(Ip2Macs[deviceNo].data, Ip2Macs[deviceNo].size * sizeof(IP2MAC));
      }
    }
    Ip2Macs[deviceNo].no++;
  } else {
...
IP2MAC *Ip2Mac(int deviceNo, in_addr_t addr, u_char *hwaddr) {
  ...
  if (ip2mac->flag == FLAG_OK) {
    ...
    DebugPrintf("dst MAC(Ip2Mac): %s\n", my_ether_ntoa_r(ip2mac->hwaddr, buf, sizeof(buf)));
    return (ip2mac);
  } else {
    DebugPrintf("Ip2Mac(%s):NG\n", in_addr_t2str(addr, buf, sizeof(buf)));
    DebugPrintf("Ip2Mac(%s):Send Arp Request\n",
                in_addr_t2str(addr, buf, sizeof(buf)));
    SendArpRequestB(Device[deviceNo].soc, addr, bcast, Device[deviceNo].addr.s_addr,
